...
static int x_left = 0;
static int y_top = yres - 1;
static int x_right = xres - 1;
static int y_bottom = 0;

static double min_depth_threshold = 0.001;
...
ReadInputs(void)
{
  ...
  if (input_normals_filename) {
    ...
    // the original nz became ny, the original ny became nz, ny and nz are swapped
    R2Grid *swap = input_normals_images[1]; input_normals_images[1] = input_normals_images[2]; input_normals_images[2] = swap;
    // the new nz (original ny) have their values negated
...
CreateSmoothnessEquations(RNSystemOfEquations& equations)
{
  ...
  cout << "Creating Smoothness Equations\n";
  // Create smoothness equations
  int count = 0;
  int total_count = 0;
  for (int iy = y_bottom; iy < y_top + 1; iy++) {
    for (int ix = x_left; ix < x_right + 1; ix++) {
      // Check if pixel is in a hole
      if (FALSE) {total_count++; continue;}
      ...
      if (ix > x_left) {
        ...
        if (w > 0) {
          ...
          e->AddTerm(-1.0, total_count,   1.0);
          e->AddTerm( 1.0, total_count-1, 1.0);
          e->Multiply(w);
      ...
      if (iy > y_bottom) {
        ...
        if (w > 0) {
          RNPolynomial *e = new RNPolynomial();
          e->AddTerm(-1.0, total_count,   1.0);
          e->AddTerm( 1.0, total_count-(x_right-x_left+1), 1.0);
          e->Multiply(w);
          equations.InsertEquation(e);
          count++;
        }
...
CreateInertiaEquations(RNSystemOfEquations& equations)
{
  ...
  if (depth_image && (inertia_weight > 0) && input_inertia_weight_image) {  // This case is ignored for now
    ...
    // Preserve depth in given depth image
    cout << "Using cropped ROI in CreateInertiaEquations" << endl;
    int total_count = 0;
    for (int j = y_bottom; j < y_top + 1; j++) {
      for (int i = x_left; i < x_right + 1; i++) {  // Instead of starting from 0 and going to the end, only use the pixels in the ROI
        RNScalar d = depth_image->GridValue(i, j);
        if ((d == 0) || (d == R2_GRID_UNKNOWN_VALUE)) {total_count++; continue;}
        RNPolynomial *e = new RNPolynomial(1.0, total_count, 1.0);
        total_count++;
        e->Subtract(d);
  ...
  else if (depth_image) {  // This case is ignored for now
...
CreateNormalEquations(RNSystemOfEquations& equations)
{
  ...
  if (input_normals_images[0] && input_normals_images[1] && input_normals_images[2] && (normal_weight > 0)) {
    cout << "Creating Normal Equations\n";
...
CreateTangentEquations(RNSystemOfEquations& equations)
{
  ...
  if (input_normals_images[0] && input_normals_images[1] && input_normals_images[2] && (tangent_weight > 0)) {
    cout << "Creating Tangent Equations\n";
    ...
    for (int iy = y_bottom; iy < y_top + 1; iy++) {
      for (int ix = x_left; ix < x_right + 1; ix++) {
        RNScalar input_nx = input_normals_images[0]->GridValue(ix, iy);
        if (input_nx == R2_GRID_UNKNOWN_VALUE) {total_count++; continue;}
        RNScalar input_ny = input_normals_images[1]->GridValue(ix, iy);
        if (input_ny == R2_GRID_UNKNOWN_VALUE) {total_count++; continue;}
        RNScalar input_nz = input_normals_images[2]->GridValue(ix, iy);
        if (input_nz == R2_GRID_UNKNOWN_VALUE) {total_count++; continue;}
        RNScalar w = tangent_weight;
        if (!normalize_tangent_vectors) w *= camera_intrinsics[0][0];
        if (input_tangent_weight_image) w *= input_tangent_weight_image->GridValue(ix, iy);
        if (w == 0) {
          // cout << "w in CreateTangentEquations is 0\n";
          total_count++;
          continue;
        }
        // // Check normal direction
        ...
        // Check that the original normal values weren't NAN (converted to 0,0,0)
        if (input_nx == 0 && input_ny == 0 && input_nz == 0) {
          total_count++;
          continue;
        }

        // Consider 4 tangent directions, order of right, up, left, down
        for (int dir = 0; dir < 4; dir++) {
          // Get image coordinates of adjacent point
          int ixA , iyA;
          ...
          if ((ixA < x_left) || (ixA > x_right)) {total_count++; continue;}
          if ((iyA < y_bottom) || (iyA > y_top)) {total_count++; continue;}

          // Compute depths
          RNPolynomial d(1.0, total_count, 1.0);
          RNPolynomial dA(1.0, total_count, 1.0);

          // Compute camera coordinates
          RNPolynomial x = d * ((ix - camera_intrinsics[0][2]) / camera_intrinsics[0][0]);  // (ix-cx) / fx
          RNPolynomial y = d * ((iy - camera_intrinsics[1][2]) / camera_intrinsics[1][1]);  // (iy-cy) / fy
          RNPolynomial xA = dA * ((ixA - camera_intrinsics[0][2]) / camera_intrinsics[0][0]);  // (ixA-cx) / fx
          RNPolynomial yA = dA * ((iyA - camera_intrinsics[1][2]) / camera_intrinsics[1][1]);  // (iyA-cy) / fy
          ...
          if (normalize_tangent_vectors) {
            cout << "Normalize tangent vectors!" << endl;
...
CreateDepthImage(void)
{
  ...
  start_time.Read();
  // int n = xres*yres;
  int n = (x_right - x_left + 1) * (y_top - y_bottom + 1);
  if (n == 0) return 0;
  cout << "Will be solving for " << n << " variables\n";
  ...
  //   return 0;
  // }
  output_depth_image = input_depth_image;
  ...
  for (int j = y_bottom; j < y_top + 1; j++) {
    for (int i = x_left; i < x_right + 1; i++) {
      ...
      if (x[x_index] < min_depth_threshold) {
        ...
        output_depth_image->SetGridValue(i, j, x[x_index]);
      }
      x_index++;
    }
  }
...
ParseArgs(int argc, char **argv)
{
  ...
  while (argc > 0) {
    if ((*argv)[0] == '-') {
      if (!strcmp(*argv, "-verbose")) print_verbose = 1; 
      ...
      else if (!strcmp(*argv, "-x_left")) { argc--; argv++; x_left = atoi(*argv); }
      else if (!strcmp(*argv, "-y_top")) { argc--; argv++; y_top = atoi(*argv); }
      else if (!strcmp(*argv, "-x_right")) { argc--; argv++; x_right = atoi(*argv); }
      else if (!strcmp(*argv, "-y_bottom")) { argc--; argv++; y_bottom = atoi(*argv); }
      else if (!strcmp(*argv, "-min_depth_threshold")) { argc--; argv++; min_depth_threshold = atof(*argv); }
  ...
  // Check validity of custom ROI
  if (x_left < 0 || x_right >= xres || y_bottom < 0 || y_top >= yres || x_left >= x_right || y_bottom >= y_top) {
    printf("Selected ROI is out of bounds\n");
    return 0;
  }
